...
class StreamingOutput(io.BufferedIOBase):
    def __init__(self):
        ...
        self.condition = (
            Condition()
        )  # Initialize the condition variable for thread synchronization
...
class Camera:
    def __init__(
        self, preview_size=(640, 480), hflip=False, vflip=False, stream_size=(400, 300)
    ):
        self.camera = Picamera2()  # Initialize the Picamera2 object
        self.transform = Transform(
            hflip=1 if hflip else 0, vflip=1 if vflip else 0
        )  # Set the transformation for flipping the image
        preview_config = self.camera.create_preview_configuration(
            main={"size": preview_size}, transform=self.transform
        )  # Create the preview configuration
        self.camera.configure(
            preview_config
        )  # Configure the camera with the preview settings
        ...
        self.stream_config = self.camera.create_video_configuration(
            main={"size": stream_size}, transform=self.transform
        )  # Create the video configuration
        self.streaming_output = (
            StreamingOutput()
        )  # Initialize the streaming output object
        self.streaming = False  # Initialize the streaming flag

    def start_image(self):
        self.camera.start_preview(
            Preview.QTGL
        )  # Start the camera preview using the QTGL backend
        self.camera.start()  # Start the camera

    def save_image(self, filename):
        metadata = self.camera.capture_file(
            filename
    ...
    def start_stream(self, filename=None):
        if not self.streaming:
            ...
            self.camera.configure(
                self.stream_config
            )  # Configure the camera with the video stream settings
            if filename:
                ...
                output = FileOutput(
                    self.streaming_output
                )  # Set the streaming output object
